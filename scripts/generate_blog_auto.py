...
def detect_watermark_text(img_path: Path) -> bool:
    """OCR 检测明显水印词。检测到底部区域，命中则判定为疑似水印。"""
    ...
    try:
        ...
        w, h = img.size
        # 重点看底部1/4区域（常见水印位置）
        roi = img.crop((0, int(h * 0.72), w, h))
        text = pytesseract.image_to_string(roi, lang="chi_sim+eng").upper()
        return any(k in text for k in WATERMARK_HINTS)
    except Exception:
        return False
...
def generate_siliconflow_image(topic_type: str, slug: str) -> str:
    ...
    prompt = IMAGE_PROMPTS.get(topic_type, IMAGE_PROMPTS["default"]) + "，无文字、无logo、无水印"
    ...
    for model_name in SILICONFLOW_IMAGE_MODELS:
        for attempt in range(1, 3):
            try:
                ...
                if detect_watermark_text(img_path):
                    print(f"[WARN] Watermark detected ({model_name}, attempt {attempt}), retrying...")
                    try:
